...
extern "C" {
  ...
  extern void send_emergency_recover();
  extern int ardrone_at_set_progress_cmdC(int32_t enable, int32_t y, int32_t x, int32_t z, int32_t yaw);
...
static int32_t xb = 0, yb = 0, yawb = 0, zb = 0, cmd_enabled = 1;
C_RESULT update_keyboard(void)
{
  ...
  if(planner->enabled)
  {
    ...
    cmd_enabled = 1 - planner->hover_final;
    //printf("cmd_enabled = %d\n", cmd_enabled);
    //ardrone_at_set_progress_cmdC(0, yb, xb, zb, yawb);
    ardrone_at_set_progress_cmdC(cmd_enabled, yb, xb, zb, yawb); //range checking done in C wrapper
  }
  ...
  */
  ...
  {
    ardrone_at_set_progress_cmdC(1, yb, xb, zb, yawb); //range checking done in C wrapper
  }
